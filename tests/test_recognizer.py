...
def test_recognizer_reads_each_day_row_once_and_keeps_ts(tmp_path, monkeypatch):
    ...
    assert len(calls) == 32
    assert [(item.day, item.waste) for item in result.collections] == [(date(2026, 7, 1), "TS")]
